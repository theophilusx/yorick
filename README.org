...
*** Namespaces
    ...
    - *theophilusx.yorick.basic* :: A collection of basic utility components:
      ...
      - render-set :: a simple component for rendering sets of data
    ...
    - *theophilusx.yorick.icon* :: components to manage display of icons
      - deficon :: define an icon map for use with the ~icon~ component
      - icon :: render a single icon
      - icons :: render a pair of icons (left and right)
    - *theophilusx.yorick.input* :: a collection of user input components
      ...
      - defoption :: define an option map for use with the ~select~ component
      ...
      - number :: an input component for numbers
    ...
    - *theophilusx.yorick.sidebar* :: a side menu (vertical) component
      - defentry :: define a sidebar menu entry
      - defmenu :: define a sidebar menu
      - sidebar :: render a sidebar from one or more menus
    - *theophilusx.yorick.store* :: various functions to manipulate Reagent atoms for data storage
    - *theophilusx.yorick.tab* :: a tabbed interface component
      - deftab :: define a tab entry
      - tab-bar :: render a tab bar from a collection of tab entries
    - *theophilusx.yorick.table* :: a component to render data in HTML tables
    - *theophilusx.yorick.toolbar* :: a basic component for rendering a toolbar
      - defitem :: define a toolbar item
      - toolbar :: render a toolbar from a collection of items
...
*** Layout   
    ...
    your layout and /Bulma/ helper classes to style the /Yorick/ components.
...
*** Managing State
    ...
    defines a global atom called ~default-store~ to store state information which
...
#+end_src
    ...
    3. Tracking state globally. The ~store~ namespace defines an atom called
       ~default-store~, which can be used to store global state information. This
       ...
       this atom. For components, like navbars, tab bars or sidebars, which use
...
** Testing
   ...
   /Yorick/ does not have an automated test suite. Testing is performed manually
   using two interactive builds included in the repository:

   - *Demo* (~demo/yorick_demo/~) :: a showcase site with live examples of every
     component, along with usage notes. This is the primary reference for
     understanding how components behave.
   - *Testbed* (~experiment/testbed/~) :: an interactive scratch environment for
     experimenting with individual components in isolation.

   If you are trying to diagnose a bug or verify behaviour, start the demo build
   and navigate to the relevant component page:
...
** License
   ...
   Copyright \copy 2020-2026 Tim Cross
